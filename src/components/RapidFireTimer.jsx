...
import './RapidFireTimer.css'

const RapidFireTimer = () => {
  ...
  useEffect(() => {
    ...
    const createBeep = (frequency = 800, duration = 0.3) => {
      ...
      gainNode.gain.setValueAtTime(0.5, audioContext.currentTime) // Increased volume from 0.2 to 0.5
  ...
  const handlePhaseComplete = () => {
    ...
    if (phase === 'loading') {
      ...
      // After preparing, start shooting
      if (currentShoot === 0) {
        // This is after initial loading - start first shot
        setPhase('shooting')
        setTimeLeft(PHASES.shooting.duration)
        setCurrentShoot(1)
      } else {
        // This is between shots - start next shot
        setPhase('shooting')
        setTimeLeft(PHASES.shooting.duration)
        setCurrentShoot(prev => prev + 1)
      }
    }
  }
  ...
  const getPhaseDisplay = () => {
    ...
    if (phase === 'preparing') {
      if (currentShoot === 0) {
        return `Prepare for Shot 1`
      }
      return `Prepare for Shot ${currentShoot + 1}`
    }
    return ''
  }
  ...
  return (
    <div className="dwelling-timer">
      <h2>Rapid Fire Timer</h2>
      <p className="sequence-info">60s Loading → 7s Prepare → 3s Shoot (Green) → 7s Prepare (Red) × 5 shots per set</p>
